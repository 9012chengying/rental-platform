...
public class BookDao {
	 ...
	 * @param num
	 */
	public void addBook(String card, String name, String type, String autho,  int num) {
		// TODO Auto-generated method stub
		Connection conn = DBUtil.getConnectDb();
		String sql = "insert into book(card,name,type,autho,num) values(?,?,?,?,?)";
		int rs = 0;
	...
	/**
	 * 获取收藏记录的全部信息，传入的条件有status，aid，表示搜索已收藏的，或者已经取消收藏的信息，aid代表当前登录用户
	 * @param status
	 * @return
	 */
	...
	/**
	 * 获取收藏记录的全部信息，传入的条件有status，1表示搜索收藏的，2取消收藏的信息
	 * @param status
	 * @return
	 */
	...
	/**
	 * 获取单个房源信息，根据传入的bid来查找，返回一个BookBean数据类型
	 * @param bid
	 * @return
	 */
	...
	/**
	 * 用户查找房源，根据输入的名称，使用like进行模糊查询，然后返回一个ArrayList数组类型
	 * @param name
	 * @return
	 */
...
//			stm.setInt(2, currentCount);
	...
	/**
	 * 房源收藏函数，根据传入bid房源id，adminbean当前登录用户的信息，在收藏记录数据表中新插入一条记录
	 * @param bid
	 * @param adminbean
	 */
